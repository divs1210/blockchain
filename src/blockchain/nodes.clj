...
(defn node-add
  ...
  (if-not (boolean (some #(= -data %) @nodes))
    (do
      (swap! nodes conj -data)
      {:message "Node inserted"
       :total-nodes @nodes})
    {:message "Node already inserted"
     :total-nodes @nodes}))
...
(defn valid-chain
  "This method is responsible for checking if a chain is valid
  ...
  (apply (fn [& x]
           (and (apply = x)
                (true? (last x))))
         (rest
          (for [index (range (count chain))
                :let [[before [current]] (split-at index chain)]]
            (= (utils/sha256hash (str (last before)))
               (:previous-hash current))))) )

(defn resolve-conflicts
  "This method loops through all our neighbouring nodes,
  ...
  (if-not (zero? (count @nodes))
    (doseq [node @nodes
            ...
            :when (and (> length (count @core/chain))
                       (valid-chain chain))]
      (core/chain-reset (vec chain)))
